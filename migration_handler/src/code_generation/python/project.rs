...
use crate::{CodeGenerator, Project, GenError, GenResult, GeneratedCode, targets};
...
impl CodeGenerator<targets::Python> for Project {
    ...
    fn aggregate_content<P: AsRef<std::path::Path>>(&self, p: P) -> crate::GenResult<GeneratedCode> {
        ...
        for schema_id in self.iter_schema() {
            ...
            let added_files = CodeGenerator::<targets::Python>::aggregate_content(schema, output_folder)?;
