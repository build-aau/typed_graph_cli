...
use super::{write_nodes_py, write_edges_py, write_node_type_py, write_edge_type_py};
use std::fs::create_dir;

impl CodeGenerator<targets::Python> for Schema<String> {
    ...
    fn aggregate_content<P: AsRef<std::path::Path>>(&self, p: P) -> crate::GenResult<GeneratedCode> {
        let schema_folder = p.as_ref().join(CodeGenerator::<targets::Python>::get_filename(self));
        ...
        write_content(self, &mut new_files, &nodes_folder, &structs_folder, &edges_folder, &types_folder)?;
        ...
        write_init(self, &mut new_files, &schema_folder, &nodes_folder, &structs_folder, &edges_folder, &types_folder)?;
...
fn write_content(
    ...
    for stm in &schema.content {
        let added_files = match stm {
            SchemaStm::Node(n) => CodeGenerator::<targets::Python>::aggregate_content(n, &nodes_folder),
            SchemaStm::Struct(n) => CodeGenerator::<targets::Python>::aggregate_content(n, &structs_folder),
            SchemaStm::Edge(n) => CodeGenerator::<targets::Python>::aggregate_content(n, &edges_folder),
            SchemaStm::Enum(n) => CodeGenerator::<targets::Python>::aggregate_content(n, &types_folder),
...
fn write_init(
    ...
    for stm in &schema.content {
        let (filename, type_name, f) = match stm {
            SchemaStm::Node(n) => (CodeGenerator::<targets::Rust>::get_filename(n), &n.name, &mut nodes_init),
            SchemaStm::Struct(n) => (CodeGenerator::<targets::Rust>::get_filename(n), &n.name, &mut structs_init),
            SchemaStm::Edge(n) => (CodeGenerator::<targets::Rust>::get_filename(n), &n.name, &mut edges_init),
            SchemaStm::Enum(n) => (CodeGenerator::<targets::Rust>::get_filename(n), &n.name, &mut types_init),
    ...
    writeln!(s, "Graph = TypedGraph[Node, Edge, imports.NodeId, imports.EdgeId, NodeType, EdgeType, {schema_name}]")?;
    writeln!(s, "")?;
    writeln!(s, "__main__ = [")?;
    writeln!(s, "    'EdgeType',")?;
    writeln!(s, "    'NodeType',")?;
...
fn write_schema_impl_rs(
    ...
    writeln!(schema_py, "class {schema_name}(SchemaExt[Node, Edge, NK, EK, NodeType, EdgeType]):")?;
    writeln!(schema_py, "    endpoint_meta: ClassVar[Dict[Tuple[EdgeType, NodeType, NodeType], int]] = {{")?;
    for e in schema.edges() {
        let edge_type = &e.name;
        for ((source, target), endpoint) in &e.endpoints {
            match endpoint.quantity.max {
                Bound::Included(i) => {
                    let i = i + 1;
    ...
    writeln!(schema_py, "        if allowed_quantity is not None and quantity >= allowed_quantity:")?;
