...
impl CodeGenerator<targets::Rust> for (&ChangeSet<String>, &Schema<String>, &Schema<String>) {
    ...
    fn aggregate_content<P: AsRef<Path>>(&self, p: P) -> GenResult<GeneratedCode> {
        ...
        let new_schema_folder = p.as_ref().join(CodeGenerator::<targets::Rust>::get_filename(self.2));
        let old_schema_folder = p.as_ref().join(CodeGenerator::<targets::Rust>::get_filename(self.1));
        
        let old_mod = changeset.old_version.to_string().replace(".", "_").to_snake_case();
        let new_mod = changeset.new_version.to_string().replace(".", "_").to_snake_case();
        ...
        for stm in &new_schema.content {
            ...
            let (folder, filename) = match stm {
                SchemaStm::Node(n) => (&nodes_folder, CodeGenerator::<targets::Rust>::get_filename(n)),
                SchemaStm::Struct(n) => (&structs_folder, CodeGenerator::<targets::Rust>::get_filename(n)),
                SchemaStm::Edge(e) => (&edges_folder, CodeGenerator::<targets::Rust>::get_filename(e)),
                SchemaStm::Enum(t) => (&types_folder, CodeGenerator::<targets::Rust>::get_filename(t)),
            ...
            let s = match stm {
                SchemaStm::Node(n) => {
                    write_node_from(n, changeset, &parent_ty)
                },
                SchemaStm::Struct(n) => {
                    write_struct_from(n, changeset, &parent_ty)
                },
                SchemaStm::Edge(e) => {
                    write_edge_from(e, changeset, &parent_ty)
                },
                SchemaStm::Enum(t) => {
                    write_type_from(t, changeset, &parent_ty)
                },
