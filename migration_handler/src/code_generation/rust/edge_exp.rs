...
impl CodeGenerator<targets::Rust> for EdgeExp<String> {
    ...
    fn aggregate_content<P: AsRef<std::path::Path>>(
        ...
        write!(s, "       id: EK")?;
        for (field_name, field_ty) in &self.fields.fields {
            writeln!(s, ",")?;
            let field_type = field_ty.ty.to_rust_type();
        ...
        writeln!(s, "        Self {{")?;
        write!(s, "           id")?;
        for (field_name, _) in &self.fields.fields {
            writeln!(s, ",")?;
            write!(s, "           {field_name}")?;
        }
        writeln!(s, "")?;
        ...
        let name_type = self.fields.get_field("name").map(|(_, ty)| ty);

        if let Some(field_value) = name_type {
            let field_type = &field_value.ty;
            writeln!(s, "")?;
...
pub(super) fn write_edges_rs<I>(
    ...
    for e in &edges {
        ...
        writeln!(
            edge,
            "impl<NK, EK, S> Downcast<NK, EK, {edge_type}<EK>, S> for Edge<EK>"
        )?;
        writeln!(edge, "where")?;
        writeln!(edge, "    NK: Key,")?;
        writeln!(edge, "    EK: Key,")?;
        ...
        writeln!(
            edge,
            "    fn downcast(&self) -> SchemaResult<&{edge_type}<EK>, NK, EK, S> {{"
        )?;
        writeln!(edge, "        match self {{")?;
    ...
    for e in &edges {
        ...
        writeln!(
            edge,
            "impl<NK, EK, S> DowncastMut<NK, EK, {edge_type}<EK>, S> for Edge<EK>"
        )?;
        writeln!(edge, "where")?;
        writeln!(edge, "    NK: Key,")?;
        writeln!(edge, "    EK: Key,")?;
        ...
        writeln!(
            edge,
            "    fn downcast_mut(&mut self) -> SchemaResult<&mut {edge_type}<EK>, NK, EK, S> {{"
        )?;
        writeln!(edge, "        match self {{")?;
        writeln!(edge, "            Edge::{edge_type}(e) => Ok(e),")?;
        writeln!(edge, "            #[allow(unreachable_patterns)]")?;
...
/// Write ./edge_type.rs
pub(super) fn write_edge_type_rs(
    ...
    I: Clone + PartialEq,
{
    ...
    for (name, _) in &e.fields.fields {
        let field_path = FieldPath::new_path(e.name.clone(), vec![name.clone()]);
...
pub(super) fn write_edge_endpoints<I: Debug>(
    schema: &Schema<I>,
    ...
    // Foreach node create an impl block with getter functions for edjecent nodes and edges
        ...
        for (dir, endpoints) in directions {
            ...
            for (end, edges) in grouped_by_end.get(&dir).unwrap() {
                ...
                let (output_edge_type, is_option) = if let Some(edge) = &only_edge_type {
                    ...
                    let quantity = edge
                        ...
                        .fold(0, |acc, (_, endpoint)| match endpoint.quantity.max {
                ...
                writeln!(s, "       Ok(g")?;
                writeln!(s, "           .get_{search_dir}_filter_edge(self.get_id(), |e| matches!(e.get_type(), {edge_types_patterns}))?")?;
                writeln!(s, "           .map(|e| (e.get_weight(), g.get_node_downcast(e.get_outer()).unwrap()))")?;
                // Cast the node into a specific type
                if let Some(_) = &only_edge_type {
                    writeln!(s, "           .map(|(e, n)| (Downcast::<_, _, {output_edge_type}<EK>, {schema_name}<NK, EK>>::downcast(e).unwrap(), n))")?;
                }
                if is_option {
                    writeln!(s, "           .next()")?;
                }
            ...
            for (endpoint, edge) in &endpoints {
                ...
                if edges_of_type > 1 {
                    ...
                    writeln!(s, "       #[allow(irrefutable_let_patterns)]")?;
                    writeln!(s, "       Ok(g")?;
                    writeln!(s, "           .get_{search_dir}_filter_edge(self.get_id(), |e| matches!(e.get_type(), EdgeType::{edge_type}))?")?;
                    writeln!(s, "           .map(|e| (Downcast::<_, _, {edge_type}<EK>, {schema_name}<NK, EK>>::downcast(e.get_weight()).unwrap(), g.get_node(e.get_outer()).unwrap()))")?;
                    writeln!(s, "       )")?;
                    writeln!(s, "   }}")?;

                // If there are no other edges we can safely cast the node to a specific type
                } else {
                    let is_option = endpoint.quantity.max == Bound::Included(1)
                        || endpoint.quantity.max == Bound::Excluded(2);
                    let return_type = if is_option {
                    ...
                    writeln!(s, "       Ok(g")?;
                    writeln!(s, "           .get_{search_dir}_filter_edge(self.get_id(), |e| matches!(e.get_type(), EdgeType::{edge_type}))?")?;
                    writeln!(s, "           .map(|e| (Downcast::<_, _, {edge_type}<EK>, {schema_name}<NK, EK>>::downcast(e.get_weight()).unwrap(), g.get_node_downcast(e.get_outer()).unwrap()))")?;
                    if is_option {
                        writeln!(s, "           .next()")?;
                    }
                    writeln!(s, "       )")?;
            ...
            for (endpoint, edge) in &endpoints {
                ...
                writeln!(s, "           .get_{search_dir}_filter_edge(self.get_id(), |e| matches!(e.get_type(), EdgeType::{edge_type}))?")?;
                writeln!(s, "           .map(|e| (Downcast::<_, _, {edge_type}<EK>, {schema_name}<NK, EK>>::downcast(e.get_weight()).unwrap(), g.get_node(e.get_outer()).unwrap()))")?;
                writeln!(s, "           .filter_map(|(e, n)| Some((e, Downcast::<_, _, {target_type}<NK>, {schema_name}<NK, EK>>::downcast(n).ok()?)))")?;
                if is_option {
                    writeln!(s, "           .next()")?;
                }
                writeln!(s, "       )")?;
