...
impl CodeGenerator<targets::Rust> for NodeExp<String> {
...
pub(super) fn write_nodes_rs(
    ...
    let name_type = nodes
        .iter()
        .map(|e| e.fields.get_field("name"))
        .fold(Some(HashSet::new()), |acc, name_field| {
            acc.zip(name_field)
                .map(|(mut field_types, (_, field_value))| {
                    field_types.insert(field_value.ty.to_string());
                    field_types
                })
        });
    ...
    for n in &nodes {
        ...
        writeln!(
            node,
            "impl<NK, EK, S> Downcast<NK, EK, {node_type}<NK>, S> for Node<NK>"
        )?;
        writeln!(node, "where")?;
        writeln!(node, "    NK: Key,")?;
        writeln!(node, "    EK: Key,")?;
        ...
        writeln!(
            node,
            "    fn downcast(&self) -> SchemaResult<&{node_type}<NK>, NK, EK, S> {{"
        )?;
        writeln!(node, "        match self {{")?;
    ...
    for n in &nodes {
        ...
        writeln!(
            node,
            "impl<NK, EK, S> DowncastMut<NK, EK, {node_type}<NK>, S> for Node<NK>"
        )?;
        writeln!(node, "where")?;
        writeln!(node, "    NK: Key,")?;
        writeln!(node, "    EK: Key,")?;
        ...
        writeln!(
            node,
            "    fn downcast_mut(&mut self) -> SchemaResult<&mut {node_type}<NK>, NK, EK, S> {{"
        )?;
        writeln!(node, "        match self {{")?;
        writeln!(node, "            Node::{node_type}(n) => Ok(n),")?;
        writeln!(node, "            #[allow(unreachable_patterns)]")?;
...
/// Write ./node_type.rs
pub(super) fn write_node_type_rs(
    schema: &Schema<String>,
    new_files: &mut GeneratedCode,
    schema_folder: &Path,
) -> GenResult<()> {
...
pub(super) fn write_node_from(
    ...
    for (field_name, _) in &n.fields.fields {
