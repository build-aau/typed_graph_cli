...
use crate::{CodeGenerator, Project, GenError, GenResult, GeneratedCode, Direction, ToSnakeCase, targets};
...
impl CodeGenerator<targets::Rust> for Project {
    ...
    fn aggregate_content<P: AsRef<std::path::Path>>(&self, p: P) -> crate::GenResult<GeneratedCode> {
        ...
        for schema_id in self.iter_schema() {
            ...
            let added_files = CodeGenerator::<targets::Rust>::aggregate_content(schema, output_folder)?;
        ...
        for changeset_id in self.iter_changesets() {
            ...
            let added_files = CodeGenerator::<targets::Rust>::aggregate_content(&(changeset, old_schema, new_schema), output_folder)?;
            new_files.append(added_files);
        }
...
fn write_mod(
    ...
    for schema in project.iter_schema() {
        ...
        writeln!(project_mod, "pub mod {};", CodeGenerator::<targets::Rust>::get_filename(schema))?;
        writeln!(project_mod, "pub use {}::{};", CodeGenerator::<targets::Rust>::get_filename(schema), schema.version.to_string().replace(".", "_"))?;
    }

    writeln!(project_mod, "pub mod imports;")?;
