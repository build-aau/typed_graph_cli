...
impl CodeGenerator<targets::Rust> for Schema<String> {
...
fn write_mod(
    ...
    writeln!(schema_mod, "mod structs;")?;
    writeln!(schema_mod, "mod edge;")?;
    writeln!(schema_mod, "mod types;")?;
    writeln!(schema_mod, "mod nodes;")?;
    writeln!(schema_mod, "mod edges;")?;
    ...
    writeln!(schema_mod, "pub mod imports;")?;
...
fn write_schema_impl_rs(
    ...
    writeln!(schema_rs, "")?;
    writeln!(schema_rs, "fn allow_node(&self, _node_ty: NodeType) -> Result<(), DisAllowedNode> {{")?;
    ...
    writeln!(schema_rs, "        match (edge_ty, source_ty, target_ty) {{")?;
...
pub(super) fn write_migrate_schema(
    old_schema: &Schema<String>,
    new_schema: &Schema<String>,
    handler: &Option<Ident<String>>,
    new_path: &String,
) -> GenResult<String> {
    ...
    for n in old_schema.edges() {
        ...
        if new_types.contains(&n.name) {
            writeln!(s, "            EdgeType::{node_type} => Some({new_path}::EdgeType::{node_type}),")?;
        } else {
    ...
    for n in old_schema.nodes() {
        ...
        if new_types.contains(&n.name) {
            writeln!(s, "            NodeType::{node_type} => Some({new_path}::NodeType::{node_type}),")?;
        } else {
